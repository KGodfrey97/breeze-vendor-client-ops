...
interface ClaimInsuranceManagerProps {
  claim: ClaimWithPatientAndProfile
}

export function ClaimInsuranceManager({ claim }: ClaimInsuranceManagerProps) {
  ...
  const fetchInsurances = async () => {
    ...
    const response = await fetch(`/api/claims/${claim.id}/insurance`, { cache: "no-store" })
  ...
  const openAddInsuranceDialog = (priority: number) => {
    ...
    setInsuranceForm({
      ...
      member_name: `${claim.first_name} ${claim.last_name}`.trim(),
      member_dob: claim.dob || "",
    })
    setIsInsuranceDialogOpen(true)
  }

  const openEditInsuranceDialog = (insurance: ClaimInsurance) => {
  ...
  const handleInsuranceSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload: Database["public"]["Tables"]["claim_insurance"]["Insert"] = {
      claim_id: claim.id,
    ...
    const response = await fetch(
      insuranceDialogMode === "edit" && editingInsuranceId
        ? `/api/claims/${claim.id}/insurance/${editingInsuranceId}`
        : `/api/claims/${claim.id}/insurance`,
      {
    ...
    if (!response.ok) {
      ...
      console.error("Error saving claim insurance:", submitError)
      setInsuranceSubmitError(submitError?.error || "Failed to save claim insurance")
  ...
  const handleDeleteInsurance = async (insurance: ClaimInsurance) => {
    ...
    const response = await fetch(`/api/claims/${claim.id}/insurance/${insurance.id}`, {
      method: "DELETE",
    })

    if (!response.ok) {
      const deleteError = await response.json().catch(() => null)
      console.error("Error deleting claim insurance:", deleteError)
      setInsuranceSubmitError(deleteError?.error || "Failed to delete claim insurance")
      return
    }
    ...
    <>
      <InfoCard title="Insurance Information" icon={FileText} className="relative">
        {isLoading ? (
          <div className="rounded-2xl border border-dashed border-muted-foreground/30 bg-muted/20 px-6 py-8 text-center">
            <p className="text-sm text-muted-foreground">Loading claim insurance records...</p>
          </div>
        ) : insurances.length > 0 ? (
                  ...
                  {insurance.priority === 1 && canAddSecondaryInsurance && (
                    <div className="flex justify-start pl-1">
                      <Button
                        ...
                        size="sm"
                        variant="outline"
                        onClick={() => openAddInsuranceDialog(2)}
                      >
                  ...
                  {insurance.priority === 2 && canAddTertiaryInsurance && (
                    <div className="flex justify-start pl-1">
                      <Button
                        ...
                        size="sm"
                        variant="outline"
                        onClick={() => openAddInsuranceDialog(3)}
                      >
                        <Plus className="mr-2 h-4 w-4" />
                        Add Tertiary
                      </Button>
          ...
          <div className="space-y-4 rounded-2xl border border-dashed border-muted-foreground/30 bg-muted/20 px-6 py-8 text-center">
            ...
            <div className="space-y-1">
              <p className="font-semibold">No claim insurance records yet</p>
              <p className="text-sm text-muted-foreground">
                Add primary claim coverage first, then secondary and tertiary plans as needed.
              </p>
            </div>
      ...
      >
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {insuranceDialogMode === "edit" ? "Edit" : "Add"} {insurancePriorityLabels[insurancePriorityToAdd]} Claim Insurance
            </DialogTitle>
            <DialogDescription>
              {insuranceDialogMode === "edit"
                ? `Update the ${insurancePriorityLabels[insurancePriorityToAdd]?.toLowerCase()} claim insurance plan for this claim.`
                : `Add the missing ${insurancePriorityLabels[insurancePriorityToAdd]?.toLowerCase()} claim insurance plan for this claim.`}
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleInsuranceSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="claim_insurance_provider">Insurance Provider</Label>
                <Input
                  id="claim_insurance_provider"
              ...
              <div className="space-y-2">
                <Label htmlFor="claim_plan_type">Plan Type</Label>
                <Input
                  id="claim_plan_type"
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="claim_policy_number">Policy Number</Label>
                <Input
              ...
              <div className="space-y-2">
                <Label htmlFor="claim_group_number">Group Number</Label>
                <Input
                  id="claim_group_number"
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="claim_payer_id">Payer ID</Label>
                <Input id="claim_payer_id" value={insuranceForm.payer_id} onChange={(e) => handleInsuranceFormChange("payer_id", e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="claim_payer_phone_number">Payer Phone Number</Label>
                <Input
                  id="claim_payer_phone_number"
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="claim_member_name">Member Name</Label>
                <Input
                  id="claim_member_name"
                  value={insuranceForm.member_name}
                  onChange={(e) => handleInsuranceFormChange("member_name", e.target.value)}
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="claim_member_dob">Member DOB</Label>
                <Input
                  id="claim_member_dob"
                  type="date"
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="claim_effective_date">Effective Date</Label>
                <Input
                  id="claim_effective_date"
                  type="date"
              ...
              <div className="space-y-2">
                <Label htmlFor="claim_termination_date">Termination Date</Label>
                <Input
                  id="claim_termination_date"
                  type="date"
            ...
            <div className="space-y-2">
              <Label htmlFor="claim_claim_address">Claim Address</Label>
              <Textarea
                id="claim_claim_address"

...
export async function PATCH(request: NextRequest, context: { params: { id: string; insuranceId: string } }) {
  try {
    ...
    return handleApiError(error, "Failed to update patient insurance")
  }
}
...
export async function DELETE(_request: NextRequest, context: { params: { id: string; insuranceId: string } }) {
  try {
    ...
    await query(`DELETE FROM patient_insurance WHERE id = $1 AND patient_id = $2`, [insuranceId, id])
    return NextResponse.json({ success: true })
  } catch (error) {
    return handleApiError(error, "Failed to delete patient insurance")
  }
}
